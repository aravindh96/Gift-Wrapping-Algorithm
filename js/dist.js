...
var drawLine = function(a, b, color,stroke = 2,id) {
    svg.append("line")
...
var buttons = {
    ...
    giftWrap: d3.select("#giftWrap"),
    remove: d3.select("#remove"),
    reset: d3.select("#reset"),
    step: d3.select("#step"),
    step1: d3.select("#step1"),
...
var reset = function() {
    ...
    buttons.giftWrap.attr("enabled", "disabled");
    buttons.remove.attr("disabled", "disabled");
    buttons.step.attr("enabled", "disabled");
    buttons.step1.attr("enabled", "disabled");
    ...
    d3.select("#console").text("Select points and click Gift Wrap once");
...
buttons.closePolygon.on("click", function() {
    if (points.length < 3) {
        ...
        if (0) {
            ...
            buttons.closePolygon.attr("enabled", "disabled");
...
buttons.triangulate.on("click", function() {
    ...
    buttons.triangulate.attr("disabled", "disabled");
    // buttons.giftWrap.attr("disabled", null);
});
var ind;
var lind = ind;
var curr_ind;
...
buttons.giftWrap.on("click", function() {
    console.log("Finding the leftmost point");

    var convexHull = [];
    // var ind;
    var point = points[0];
    var leftMost = point.x;
    len = points.length;
    ...
    //   var maxorient = 0;
    //   for (var j=0; j<len; j++){
    //     var last = convexHull.length-1;
    //     drawLine(points[ind],points[j],"red",0.5,"redline")
    //     drawLine(points[j],points[curr_ind],"red",0.5, "redline")
    //     if(orient(points[ind],points[j],points[curr_ind]) < 0){
    //       maxorient = orient(points[ind],points[j],points[curr_ind]);
    //       curr_ind = j;
    //     }
    //   }
    //   ind = curr_ind;
    //
    //   var last = convexHull.length-1;
    //   drawLine(convexHull[last],points[ind],"blue","blueline")
    //   // d3.select("svg").remove();
    //
    //
    // }while(ind!=lind)
    console.log(convexHull,"CH");

    d3.select("#console").text("Keep Clicking Step to see the convex hull be formed");
    buttons.giftWrap.attr("enabled", "disabled");
...
var inner_loop = function(){
  console.log(j,len,"hello")
  if(j<len){
    var last = convexHull.length-1;
    drawLine(points[ind],points[j],"red",0.5,"redline")
    drawLine(points[j],points[curr_ind],"red",0.5, "redline")
